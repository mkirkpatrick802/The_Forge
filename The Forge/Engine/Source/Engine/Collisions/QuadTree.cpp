...
{
    glm::vec2 halfSize = _size * .5f;

    _children[0] = std::make_unique<QuadTree>(_depth + 1, _position, halfSize);                                                     // Top-left
    _children[1] = std::make_unique<QuadTree>(_depth + 1, glm::vec2(_position.x + halfSize.x, _position.y), halfSize);     // Top-right
    _children[2] = std::make_unique<QuadTree>(_depth + 1, glm::vec2(_position.x, _position.y + halfSize.y), halfSize);     // Bottom-left
    _children[3] = std::make_unique<QuadTree>(_depth + 1, _position + halfSize, halfSize);                                          // Bottom-right
